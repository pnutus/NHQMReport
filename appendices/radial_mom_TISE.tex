...
\begin{eq}
  \int \rd^3 \vec{k}' \bra{\vec{k}} H \ket{\vec{k}'} \Phi(\vec{k}') 
  &= 
  \frac{k^2}{2\mu}\phi(k)Y_l^m(\Omega_k) + \int \rd^3 \vec{k}' \bra{\vec{k}}V(r)\ket{\vec{k}'} \phi(k') Y_l^m(\Omega_{k'}) 
  \\
  &=
  \frac{k^2}{2\mu}\phi(k)Y_l^m(\Omega_k) + Y_l^m(\Omega_k) \int \rd k' \, k'^2 \phi(k') V(k,k')
  \\
  &=
