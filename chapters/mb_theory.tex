...
\begin{eq}
  ...
  0.
\end{eq}
This can also be understood through the \emph{Pauli Principle}, which says that two identical particles cannot exist with the same quantum numbers. The above equation can now be interpreted as when adding $\alpha$ to a state already containing it, it seize exist.
...
\end{eq}
\todo[inline]{greek letters outside sums?}
...
The following section will thus introduce the concept of angular momentum coupling. \todo{refer to souhonen or somewhere where this coupling is satisfactorily explained}
...
This is equivalent to the entire system being symmetric under rotation, while $\vec{J}_1$ and $\vec{J}_2$ will only be conserved if you can arbitrarily rotate one of the particles without affecting the solutions. 
This is not possible if you study directly interacting particles.
It is then convenient to switch to a basis where the total angular momentum is well defined, but the individual momenta are not. \todo{new  notation, $;$ is new and confusing.}
...
It is possible to show that $J^2$, $J_z$, $J_1^2$ and $J_2^2$ commute with each other. 
This means that one may choose a basis where $J$, $M$, $j_1$ and $j_2$ are simultanously well-defined. 
However, this is not the case with $m_1$ and $m_2$, and you neccesarily have to form a linear combination of these,
\begin{eq}
  \pket{E_1j_1, E_2j_2; JM} = \sum_{m_1, m_2} c_{m_1 m_2} \pket{E_1j_1m_1, E_2j_2m_2} .
\end{eq}
We have now introduced the new basis $\pket{E_1j_1, E_2j_2; JM}$. 
The notation is meant to read as: the first particle is described by the quantum numbers $E_1 j_1$, the second particle with $E_2 j_2$, and  they together have a total angular momentum $JM$.
...
We have thus significantly decreased the dimension of the matrices that need to be computed and diagonalized. 
 ...
 The transformation is done with the coefficients $c_{m_1 m_2}$, formally written as \todo{take on one value? please elaborate}
\begin{eq}
  c_{m_1 m_2} = \pbraket{j_1 m_1, j_2 m_2}{JM},
\end{eq}
known as the \emph{Clebsch-Gordan coefficients}. \todo{cite CG?} There are known expressions for these, and values can be found in standard tables or calculated in a fairly straigth-forward way. Suhonen\cite{suhonen} gives a detailed discussion of this.
...
We can write this result formally as 
\begin{eq}
  \ket{ab; JM} 
  &= 
