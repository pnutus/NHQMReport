...
Because the antisymmetric states change sign on exchange of two particles, should two particles be in the same state, the state will become 0. \todo{Is this really why?} Thus, no particles can occupy the same state and the Pauli principle is satisfied.
...
We can do this by requiring the sp states to always appear in the same order in the ket. If they aren't, we permute two sp states at a time until the correct ordering is reached \todo{"double count"?}
\begin{eq}
