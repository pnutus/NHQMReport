...
Even if both single-particle states are the same in the coupled basis $E_1 = E_2 = E$, we will still sum over all $m_1$, $m_2$ in the ket $\ket{E m_1 E m_2}$, meaning that we will \emph{not} order our states in that calculation. Instea \cref{eq:one-body_matrix_elements} will read
\begin{eq}
