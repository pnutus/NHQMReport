...
Instead we focus on the one-body term, which was treated in \cref{eq:one-body_matrix_elements}. There is a subtle detail, however, that needs to be addressed.

Even if both single-particle states are the same in the coupled basis $E_1 = E_2 = E$, we will still sum over all $m_1$, $m_2$ in the ket $\ket{E m_1 E m_2}$, meaning that we will \emph{not} order our states in that calculation. Instead \cref{eq:one-body_matrix_elements} will read
\begin{eq}
...
avoided by letting the sum run over just the single-particle states present in the bras and kets.
