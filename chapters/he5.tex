When we move on to the problem of the \He{5} nucleus, we view 
it as an alpha particle (\He{4}) interacting with a valence neutron.
...
\end{eq}
A visualisation of this potential can be seen in \cref{fig:woods-saxon}. \todo{Should we put en extra figure of the potential here?}
...
be seen as the range of the potential. \todo{d, r0? isn't r0 range?} We see that the spin-orbit coupling 
...
radial equation.\todo{Explicitly describe the reduction to effective 
one-body problem?} The implementation of \He{5} will thus be the same 
as for the hydrogen atom, except for the new potential being very different because of its short range.\todo{Hydrogen  atom, if we still have it.}

\section{Convergence}
\todo{Is convergence even important?}
We can solve the problem using either HO expansion or discretizised momentum space. A comparison of performance between the methods for the hydrogen atom and \He{5} problems is shown in \cref{fig:HO vs mom}.
