...
Other doubly magic light nuclei, such as \ce{^{16}O} and its isotopes, have been studied using methods similar to ours\cite{gamow_shell_model_2008}, but we will restrict ourselves to Helium nuclei. \todo{Helium capital H? is it a name?}
...
Furthermore, we see that the energies for the others strictly decrease when the range of the basis increases, further showing their unbound nature.\todo{why does this show unbound nature? (last sentence)}
...
\todo{where?}We see that we have a background of wavefunctions peaking at different values of $k$. Recall the relation between wavefunctions in position and momentum representation, \cref{eq:radial wavefunction}
\begin{eq}
  R(r)=i^l\sqrt{\frac{2}{\pi}} \fint[0][\inf]{k} k^2 \phi(k)j_l(kr).
\end{eq} 
\todo{another we see}We see that a wavefunction peaking at a single value $k_i$ corresponds to a radial wavefunction
\begin{eq}
