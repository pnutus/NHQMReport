...
\begin{document}
\todo{main scope two body nuclear problem, use he5 as concrete example}
\chapter{The Nuclear Two-Body Problem}
\label{cha:two-body}
...
We choose the \He{5} nucleus as a concrete example because it can be modeled as an alpha particle, that is a very stable double magic nucleus, with an orbitting neutron. 
...
\tikzsetnextfilename{wavefunction-50MeV}
  ...
  \caption{Two solutions to the Woods-Saxon potential that display different behavior for different depths $V_0$, using the plane wave expansion.} 
...
\todo{Move real contour to next chapter}
