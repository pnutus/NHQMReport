...
\todo{redo figure different $V_0 = 47$}
...
Solving the Schrödinger equation in the momentum basis gives us momentum eigenfunctions $\phi(k)$, presented in \cref{fig:he5_eigvecs}.
...
This is documented in \cref{fig:resonance wavefunction}, where the radial probability distributions $r^2|R(r)^2|$ are plotted for the widest momentum wavefunction together with an arbitrary unbound state. 
...
We have studied the \He{5} nucleus and found that it does not support any bound states. 
Instead we have found continuum states with one solution standing out.
This unique state show quasi-bound properties and we suspect it to be the \He{5} ground state resonance. 
Resonances are however best described by complex energies, and the basis we have been working in only give real solutions.
We need a new framework---the \emph{complex-momentum basis}---which will be treated in the next chapter.
