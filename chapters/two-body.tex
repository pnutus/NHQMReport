...
For the mean-field potential we will use the established Woods-Saxon potential visualized in \cref{fig:woods-saxons} and given by \todo[inline]{comma?}
\begin{eq}
	V(r)=
...
If we add nucleons to a doubly magic nuclei, we can thus treat it as a monolithic \emph{core}, interacting with valence neutrons through the mean-field only. \todo[inline]{heter det så? fint skrivet}
...
We see from [ref?] that the $p_{3/2}$-wave will get a negative net contribution from the total spin-orbit term, also shown in \cref{fig:woods-saxons}. \todo[inline]{ref}
...
\todo{jag skulle vilja ha något generellt HO först, annars blir det bara ett stycke om range typ}
The \emph{range} $r_0$ of the HO potential determines the rate at which the potential increases, and thus the range of the HO eigenfunctions. 
...
This is a sign that there is a solution localized in this region.\todo[inline]{hur fasen vet vi det?}
...
There is a background of wavefunctions peaking at different values of $k$, and, once again, one of the functions stand out as lower and wider. Let us first discuss the peak-shaped wavefunctions.
...
A free particle is practically unaffected by the potential term of the Hamiltonian and since it is an eigensolution to the kinetic energy operator. 
Thus we expect most of our solutions to be only marginally perturbed basis states. \todo[inline]{is this clearer or just redundant ramble?}
Hence, the fact that we find unbound solutions corresponding to each value of $k$ used in the discretization hints at the continuous nature of these states.
...
As we decrease the depth below a certain threshold, the potential well no longer supports the bound state ($E > 0$), but the wavefunction is still localized. \todo[inline]{styrkemarkör? jag gillade inte fairly}
This is the region of the \He{5} depth (\SI{47}{MeV}) where we should find the resonance.
When decreased even further the wavefunction starts to look similar to the unbound states. \todo[inline]{osäker syftning?}
...
\centering{
  ...
  \caption{
    ...
    There is still a clearly localized state for $V_0 = \SI{47}{MeV}$, but at $V_0 = \SI{40}{MeV}$ it is practically indistinguishable from the continuum states.
  } 
