...
\todo[inline]{should the ...looking for a resonance. be here?}
...
For the mean-field potential we will use the established Woods-Saxon potential, visualized in \cref{fig:woods-saxons}.
The Woods-Saxon potential with a $l-s$ couplingis is a standard shell model in nuclear physics \cite[suhonen], given by
\todo[inline]{check last sentence}
\begin{eq}
	V(r)=
...
This is the same state that showed some odd behaviour in the HO-basis, but let us first discuss the peak-shaped wavefunctions.
...
\begin{figure}[h]
  ...
  \caption{\He{5} momentum probability distributions. The resonance is significantly wider than the surrounding states. We can see a trend toward lower and wider states around \SI{1}{fm^{-1}}, but it is merely a numerical peculiarity stemming from the normalization and the fact that the unique state ´´steals" a peak that has to be shared among the other ones.} 
  \label{fig:real_momentum_wavefunctions}
\end{figure}

%%%%%%%%%%%%%%%%%%%%%%%%%WF real/complex end
\todo[inline]{fix plot legend.}
\todo[inline]{check figure text if it is good.}
...
\todo{Make it clear the unique state is the same in all these cases!}
...
At \SI{47}{MeV}, which roughly corresponds to \He{5}, we can still identify a unique state. 
...
\centering{
  ...
  \caption{
    ...
    There is still a clearly localized state for $V_0 = \SI{47}{MeV}$, the depth of \He{5}. At $V_0 = \SI{40}{MeV}$ the unique state is almost indistinguishable from the other continuum states.
  } 
