...
\todo[inline]{Check comments iv, vii, ix from Jimmy and read through. PONTUS}
...
Such a system is a called a \emph{closed} quantum system, since the number of particles is conserved and the particles localized in a finite region. 
...
They appear in the continuum, like an unbound state.
\todo[inline]{check sentences. PONTUS}
Its wavefunction is localized for a finite amount of time, as opposed to a bound state that stays forever the same.
Perhaps most curious is that the norm of the resonance state decreases exponentially with time.\todo{how do whe know this?}
...
\todo[inline]{We should mention the parallel to tunneling and radioactive decay. -or should we?}
...
\todo{Introduce that eigenvalues to the hamiltonian are energies, or can we take this for granted?}
Hermitian operators can only have real eigenvalues, and are thus insufficient for treating resonances.

\todo{aim?}The aim of this thesis is to present methods for dealing with the complex energies that describe resonances. To demonstrate the methods, we will use 
the simple nuclear systems \He{5} and \He{6}, where the ground state of \He{5} and the exite states of \He{6} are unbound and the ground state of \He{6} beeing an example of a \emph{Borromean} system.
