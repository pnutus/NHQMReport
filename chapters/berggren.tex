...
\begin{eq}
  ...
  E\phi(k) \, .
\end{eq}
Using a complex momentum basis effectively means that we instead\todo{instead of what?} evaluate the integral along a complex contour. 
...
An example of a contour is seen in \cref{fig:triangle_contour}, marking each point where the integrand is evaluated. Note the concentration of points near the ends of each segment, resulting from the GL-quadrature.\todo{GL?}
