...
\label{cha:berggren}
As we argued in the introduction, resonances can be properly described with a complex energy, $E=E_0-i\frac{\Gamma}{2}$. We may relate the energy of a system to a momentum, according to
\begin{eq}
...
\end{eq}
Having a complex energy is in this sense equivalent to being described with complex momenta. Because of this, if we want to explicitly find the resonant states we need to extend the plane wave basis into the complex momentum plane. To clarify what this means, consider again the momentum space Schrödinger equation, \cref{eq:radial mom space TISE}:\todo{Momentum space Schrödinger equation? Plane wave SE? Spherical wave SE?}
\begin{eq}
  \frac{k^2}{2\mu}\phi(k) + \fint[0][\inf]{k} k'^2 V(k,k') \phi(k') 
  &=
  E\phi(k) \, .
\end{eq}
By using a complex momentum basis it is effectively meant that we evaluate the integral along a complex contour. In 1968, Berggren\cite{berggren} first showed that the solutions that are obtained using this scheme -- bound states, resonant states and the non-resonant continuum -- form a complete orthonormal basis. This is a major result, meaning that resonances are included as proper solutions to open quantum systems. This is of importance later, where we consider systems of multiple particles interacting. In this Chapter, we will instead focus on using complex momentum contours to find the resonances of \He{5}. 

We will start by mentioning some of the key features of the theory behind the complex momentum representation. However, we will in no way attempt to give a comprehensive or rigorous introduction to it, since the mathematical nature of the theory is beyond the scope of this report.
...
To find the resonance, a mathematical quirk requires that the complex $k$ contour passes below the pole corresponding to the resonance. A contour like this is visualized in \cref{fig:berggren contour}. Note that the right hand side of the plane is mirrored on the left. Solutions on the right side are to be interpreted as 'ingoing' solutions, while solutions on the left side can be interpreted as 'outgoing'. However, use of this closed contour is mostly a mathematical technique to prove the completeness relation. In fact, symmetry in the situation lets us restrict ourselves to the segment between $k=0$ and $k=+\infty$. Note that our original momentum space equation corresponded to a contour along the real line, $k \in [0,\, \infty]$.
