...
\end{eq} \todo{Perhaps add a discussion/motivation of this ''definition''?}
...
A curious result following from this procedure is that the ordinary scalar product,
\begin{eq}
  \braket{\phi}{\psi} = \fint[0][\inf]{k} k^2 \conj{\phi(k)}\psi(k)
\end{eq}
is replaced in favor for
\begin{eq}
  \braket{\phi}{\psi} = \fint[0][\inf]{k} k^2 \phi(k)\psi(k)
\end{eq} 
without the conjugation. This also follows over to the norm. 
...
\Cref{fig:mom wavefunctions}\todo{ref ref} shows the momentum distribution of an arbitrary unbound solution and the resonance. 
As is expected\todo{as expected?}, the unbound solution basically correspond to one definite momentum. 
       ...
       \begin{tikzpicture}
         \begin{axis}[
         	...
         	\addplot+[color = red, only marks,very thick] table [x index =2, y index =3] {../figures/poles(contour)/square.data};
         	    \addlegendentry{Pole position}
         	\addplot+[color = gray, no marks, very thick] table [x index =0, y index =1] {../figures/poles(contour)/square.data};
             \addlegendentry{Contour}
       ...
       \begin{axis}[
		   ...
		   xlabel=Re $k/\b{\si{fm^{-1}}}$]
         	...
         	\addplot+[color=red, only marks, very thick] table [x index =2, y index =3] {../figures/poles(contour)/triangle.data};
         	    \addlegendentry{Pole position}
         	\addplot+[color=gray, no marks, very thick] table [x index =0, y index =1] {../figures/poles(contour)/triangle.data};
			             \addlegendentry{Contour}
