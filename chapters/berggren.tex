  ...
  \tikzsetnextfilename{berggren_contour}
    \begin{tikzpicture}[scale = 2.7]
      ...
      \draw[very thick, mid arrows] (0,0) 
         -- (0.25, -0.25) 
         -- (0.5, 0) node[pos=0.5, below right] {$L_+$} 
         -- (1, 0);
...
An example of a contour is seen in \cref{fig:triangle_contour}, marking each quadrature point where the integrand is evaluated. Note the concentration of points near the ends of each segment, resulting from the Gauss-Legendre quadrature.
...
\todo{figure markers, romb, kvadrat, cirkel, triangel}
\section{\He{5} Revisited}
\todo{structure PONTUS}
Armed with with our new contour we can continue our study of the \He{5} system. 
...
\todo{more focus on numerical, relate to something PONTUS}
...
\caption{\He{5} momentum probability distributions for a real and complex contour. The resonance is more prominent when solving along the complex contour but is significantly wider than the surrounding states in both cases.} 
\label{fig:complex_mom_wavefunctions}
\end{figure}

\todo{figurtext! zebra numeriskt SPILL}
...
We may now proceed to fit our model of \He{5} to experimental data of the $p_{3/2}$ and  $p_{1/2}$ resonance provided by \cite{tunl}. Experimental data and simulated values are shown in \cref{tab:resonances}

\todo{fix citation OLA}
\todo{Insert simulated values and parameters in table}
\todo{Is the table ok?}


\begin{table}
\caption{\label{tab:resonances} Experimentally obtained values\cite{tunl} for the \He{5} resonances, 
...
\begin{figure}
  ...
  \caption{\He{5} momentum solutions for different contours. The resonance is located at $0.1731, -0.03558i$ for both contours. we have used $V0 =$ \SI{47}{MeV} with $k_{max} =$ \SI{5}{fm^{-1}} with 60 points on both contours.} 

  \label{fig:pole(cont)}  
\end{figure}

\todo{triangle before square SPILL}
