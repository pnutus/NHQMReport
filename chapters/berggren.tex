...
A sample contour is visualized in \cref{fig:berggren contour}. Note that the right hand side of the complex plane is mirrored on the left. \todo{elaborate further?}
Solutions on the right side are to be interpreted as 'ingoing' solutions, while solutions on the left side can be interpreted as 'outgoing'. \todo{wat? right/l half plane? 'going'? I don't understand}
