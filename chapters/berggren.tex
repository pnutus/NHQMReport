...
\end{figure}
\todo{this has been slightly updated. Please read through it.}
...
However, doing this leads to a new scalar product (and thus norm) that must be used: \todo[inline]{kanske ha den nya först för det är det man förväntar sig innan man hinner läsa nästa mening? spill vill prata om det här}
\begin{eq}
...
The solutions will be examined in a similar fashion to \cref{cha:two-body}, using this new tool. \todo[inline]{this otydligt, tools of the berggren theory?}
...
\todo[inline]{invecklade care must... bättre att göra den lite längre men mycket simplare we must take...}
...
We expect the position of the resonance poles to be fairly close to the experimental data in \cref{tab:}. We then choose a triangular contour that extends below the expected position. \Cref{tab:contours} presents the poles and the corresponding contours. \todo[inline]{cont. used?}
...
We solve the Schrödinger equation using the contours and represent the energy solutions by their momenta $k=\sqrt{2\mu E}$. The result for the $p_{3/2}$ wave is shown in \cref{fig:triangle_contour}. We see that most solutions follow the contour, corresponding to non-resonant continuum states, similarly to the real case. \todo[inline]{synd att detta inte är sant}
...
In \cref{fig:rect_contour}, we have used a rectangular contour instead, yet the pole is completely stable. \todo[inline]{a rect. cont is instead used?}
We also found that we could vary the depth of the contour to some extent without the resonance pole moving, but if we went too far down, the matrix element integrals started to diverge. \todo[inline]{gillar inte riktigt formuleringen, lite informellt}
...
\end{center}
\end{table}
\todo{Table ok? Units in the corner or in caption?}
