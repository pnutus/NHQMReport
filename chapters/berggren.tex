...
\todo{fill in table, contours are NOT CORRECT VINCENT}
\begin{table}[H]
  \begin{tabular}{l|c|c}
...
\begin{figure}
  ...
  \caption{\He{5} momentum solutions for different contours. The resonance is located at $k = \SI{0.173 - 0.0355i}{fm^{-1}}$ for both contours. We use $k_{max} =$ \SI{5}{fm^{-1}} with 60 points on both contours.} 
...
Having studied the solutions and ensured that we have found the resonances, we now proceed to fit our model of \He{5} to experimental data. The data, taken from \cite{tunl}, is presented in \cref{tab:resonance_data}.
