...
The integral to be evaluated along the contour is in the radial momentum space Schrödinger equation
\begin{eq}
...
\begin{figure}[p]
  \centering{
   ...
   \caption{Momentum solutions for the \He{5} $p_{3/2}$ wave for different contours. The resonance is located at $k_r = \SI{0.173 -0.0356i}{fm^{-1}}$ for both contours, truncated at the same $k\sub{max} = \SI{5}{fm^{-1}}$ and using 60 points.} 
\label{fig:pole(cont)}  
\end{figure}
...
\todo{är all denna text verkligen nödvändig?}


\todo{improve corner on this}
\begin{figure}
  \centering
  \begin{tikzpicture}
    \begin{axis}[
...
As the potential well grows even less attractive, we see that both the real part $E_0$ and imaginary part $\Gamma$ of the resonance energy increases.
\todo{vad händer sen?}
\todo{mer tolkning?}
