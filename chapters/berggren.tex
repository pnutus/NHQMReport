...
In fact, this is correct, and was proven in 1968 by Berggren \cite{berggren}. \todo{no cite.}
...
\end{eq} 
without the conjugation. \todo{Does this mean anything? impacts? etc}
...
An example of a contour is seen in \cref{fig:triangle_contour}, marking each quadrature point where the integrand is evaluated. Note the concentration of points near the ends of each segment, resulting from the Gauss-Legendre quadrature (see \cref{app:gauss-legendre}).\todo{no cite.}
  ...
  \begin{tikzpicture}
    \begin{axis}[
      ...
      only marks,
		...
		legend style={at={(0.8,0.7)}, anchor=north,legend columns=1},
      ]
    ...
    \end{axis}
  \end{tikzpicture}
  \caption{Solutions to \He{5} along a complex contour. Notice the resonance in the middle of the triangle. In this figure the following parameters were used: ...}
  \label{fig:verification}
\end{figure}
\todo[inline]{Mention something about solutions in second figure?}
\todo[inline]{Why doest the solutions follow the mesh points if they are free?}
%%%%%%%%%%%%%%%%%%%%%%%%%%VERIFICATION end
...
\begin{figure}[H]
\centering{
	\subfloat[Momentum space wavefunctions using a real contour.]{
...
\begin{figure}[H] %this figure needs to be nudged a little bit to the left
   \centering{
	 ...
	 \subfloat[Real Contour \SI{-50}{MeV}]{
		 \includegraphics[page=2]{../figures/poles(realcontour)/poles.pdf}
	 }
   }
