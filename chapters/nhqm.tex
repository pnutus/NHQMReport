...
Armed with the tools listed above we can now start to study the resonance and the behavior around it in a better way.
The primary aspect of the non-hermitian formulation is that we now can localize the resonance.
This is important since we fix our basis using an experimental value on the resonance, which we adapt our potential after.
Another way to see the resonance is to follow its convergence or path as we vary the potential depth \cref{fig:pole(V0)}.
We also noticed that it was no longer the state with lowest (real) energy that were interesting, instead we had to plot many of the wavefunctions and observe their behavior to be able to detect any resonance states.
