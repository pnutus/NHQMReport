...
The sought value is as usual \unit{13.6}{eV} for the Hydrogen atom binding energy.
We see that the plane-wave expansion converges faster than the harmonic oscillator, but towards the wrong value.
This is because the momentum space expansion not support the use of non-finite potentials.
The potetntial of the hydrogen atom is the famous school-book example:
\begin{eq}
	V(r)=
	-\frac{e^2}{4\pi\epsilon_0r}.
\end{eq}
This do not converge towars a finite number as $r->0$, which means that a small deviation not surprising.
