...
In order to perform basis expansion on a computer, we need to consider the numerical aspects of the problem. This includes truncation of the basis eigensolver algorithms, numerical integration and reducing the size of the matrix.

We truncate the infinite $\ket{nlm}$ and $\ket{k}$ bases, i.e. we only include a finite number $N$ of states in the basis
\begin{eq}
  \sum_{n'=0}^N H_{nn'} \psi_{n'} = E \psi_n.
\end{eq}
