...
If we considered three-dimensional systems with arbitrary potentials, the matrix size would grow very fast and would be infeasible to calculate.
...
A rather involved calculation (see \cref{app:plane_wave_TISE}) shows that the Schrödinger equation can be written as
\begin{eq} 
...
The included states will be the $N$ first ones, since these has the biggest overlap with the actual wavefunctions. \todo{Questionable? OLA}
