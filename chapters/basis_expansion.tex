...
However, the Hamiltonian written as in \cref{eq:TISEpos}, is not suitable for numerical calculations. \todo[inline]{cite/explain}
...
We will not expand our problems in the position basis, but it will still be important, since it is the only basis in which we can express the potentials.\todo{Which potentials?}
...
Their corresponding operators, $L^2$ and $L_z$, commute with the Hamiltonian $H$, \todo{and will}which will result in a factor of $\delta_{ll'}\delta_{mm'}$.
...
\begin{eq}
  \sum_{n'} \bra{nlm} H \ket{n'lm} \psi_{n'lm} = E\psi_{nlm}.
\end{eq}
We now have a matrix equation, but we need to find the matrix elements $\bra{nlm} H \ket{n'lm}$. These require some calculations (see \cref{sec:HO matrix elements} for the details) and the result is \todo{are we stringent in the use of compute, calculate?}
\begin{eq}
...
\end{eq}
$r_0 = \sqrt{\hbar/\mu\omega}$ is the range and $L_\mu^\nu(x)$ are the generalized Laguerre polynomials.
...
\label{sec:mom_space}
\todo{Relation to momentum space $p = \hbar k$}
...
\begin{eq} 
  ...
  E\phi(k) \, ,
\end{eq}
where $\phi(k)$ is the radial part of the momentum space wavefunction, \todo[inline]{textlimmet saknas här, flödet blir konstigt}
\begin{eq}
  V(k,k') 
  &= 
...
\begin{align}
  ...
  V_{ij} &= \frac{2}{\pi} \int_0^\infty \rd r \, r^2 V(r) j_l(k_i r) j_l(k_j r) \label{eq:potential matrix}
\end{align}
...
\begin{eq}
  ...
  \label{eq:discrete radial wavefunction}
\end{eq}
...
\pgfplotstableread{../figures/convergence/hydrogen_convergence.data}\Hconv
\begin{figure}
\centering	
\begin{tikzpicture}
    \begin{axis}[
