...
\begin{eq}
  ...
  H \ket\psi = E \ket\psi,
\end{eq}
commonly written in the \todo{remove the?} position basis as \todo{Position or coordinate?}
\begin{eq}
  \label{eq:TISEpos}
...
\end{eq}
The complete bases we will use in this report are the \emph{position basis} $\ket{\vec{r}}$, the \emph{momentum basis} \todo{should we call it momentum basis?} $\ket{\vec{k}}$, the \emph{harmonic oscillator basis} $\ket{nlm}$ and the elusive \emph{Berggren basis} \cite{berggren}. All these bases are orthonormal, i.e. all the basis vectors satisfy 
\begin{eq}
