...
However, the Hamiltonian written as in \cref{eq:TISEpos}, is not suitable for numerical calculations. \todo{cite/explain}
...
Basis expansion is how we make sense of the abstract Hilbert spaces, operators and state vectors of Quantum Mechanics (QM). \todo{is this the only place where we mention Hilbert space?}
...
\begin{eq}
  ...
  E \ket\psi.
\end{eq}
By closing \cref{eq:lincomb} with $\bra{n}$ on the left side and use orthonormality, we see that $\braket{n'}{\psi} = \psi_{n'}$. Now we close \cref{eq:expand1} with $\bra{n}$ on the left \todo{passive clause -> active. :S}
\begin{eq}
  \label{eq:expand2}
...
\section{Spherical Symmetry} \todo{up to date with SDI?}
...
This is sufficient for our purposes and, indeed, most common QM problems. \todo{why?/cite/explain}
...
\begin{eq}
  \sum_{n'} \bra{nlm} H \ket{n'lm} \psi_{n'lm} = E\psi_{nlm}.
\end{eq}
We now have a matrix equation, but we need to find the matrix elements $\bra{nlm} H \ket{n'lm}$. These require some calculation (see \cref{sec:HO matrix elements} for the details) and the result is \todo{are we stringent in the use of compute, calculate?}
\begin{eq}
...
\label{sec:mom_space}
\todo{mention that we move to momentum space?}
...
\begin{eq}
  ...
  E\Phi(\vec{k}) \, .
\end{eq}
This three-dimensional equation is generally unsolvable in practice, but we can simplify it using the fact that we have a central problem. \todo{in practice}
...
\end{eq}
and $j_l(kr)$ are the spherical bessel functions of order $l$. \todo{start a sentance with a variable?} $\phi(k)$ is related to the real space radial wavefunction by
\begin{eq}
...
In order to perform basis expansion on a computer, we need to consider the numerical aspects of the problem. This includes truncation of the bases, matrix size reduction, numerical integration and eigensolver optimizations. \todo{bases/basis? we truncate the base(s) by only using so many basis states. syftning}
...
\todo[inline]{Convergence plots here?}
...
We calculate the matrix elements $H_{ij}$ with \cref{eq:HO matrix elements,eq:plane wave matrix elements}, carrying out the integrals with the Gauss-Legendre quadrature rule (see \cref{app:gauss-legendre}) and setting the upper limit to a finite number.\todo{refer to future eqs without comment?}
...
\subsection{Plane Wave Basis Discretization and Symmetrization}
...
With this approximation the Schrödinger equation may be written as \todo{approximation? discretization?}
\begin{eq}
...
\end{align}
\todo{$\phi_i$ notation might be bad... use $\phi{k_i}$ instead to free up indexes}

Because of the $k_j^2$ in the second term of the matrix elements 
(\cref{eq:mom matrix}), the $H_{ij}$ matrix will not be symmetric. 
Working with a symmetric matrix will be faster, hence we perform \todo{why?/explain more}\todo{where do we mention the need for symmetrized matrix in he6?}
the transformation
\begin{eq}
  \phi_i &\mapsto
...
\section{Verifying with the Hydrogen Atom} \todo{where to put this?}
