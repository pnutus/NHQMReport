...
However, the Hamiltonian written as in \cref{eq:TISEpos} is not optimal for computation. \todo{why?} \todo[inline]{Why not position basis PDE??}
...
We limit ourselves to spherically symmetric systems, i.e. systems with a potential $V(r)$ that only depends on the distance $r$ from the origin. \todo{boundary of this project, move to introduction?}
...
\todo{Do we introduce this earlier, since we use the word since?}
...
The included states will be the $N$ first ones, since these have the biggest overlap with the actual wavefunctions. \todo[inline]{Questionable? OLA}
