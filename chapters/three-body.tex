...
The study of this interaction at the nuclear level is therefore an active field of research \cite{living_on_the_edge}.
...
The SDI can be expanded into multipole radial components
\begin{eq}
...
The parameters of our interactions will be fit to the known $0^+$ ground state of \He{6}. The parameters are then used to make a prediction for the excited $2^+$ state---a resonance. To find the resonance, we have to include the resonance of \He{5} in the sp basis. 

\subsection{Using a Gaussian Interaction}

We began bysetting the range $R$ of the gaussian potential to the same value as the Woods-Saxon range, \SI{2}{fm}. The strength $V\sub{GI}$ was then fitted to reproduce the \He{6} ground state.
\cref{tab:he6_gaussian} summarizes the results of this investigation. 

It is clearly seen that the Gaussian interaction is unsuitable to describe this system. 
It is degenerate in $J$, meaning there is no distinction between the ground and excited states.
It is possible to fit $V_0$ to reproduce the ground state energy. However, with the range $r_0$ set to \SI{2}{fm}, it does not produce a resonance.
