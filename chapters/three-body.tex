...
\begin{figure}[h]
  ...
  \newcommand{\circrad}{2}
  \centering{
  \subfloat[The \He{6} system]{
    \label{fig:He6}
    \includegraphics[width=0.45\textwidth]{../front/chalmers-master/helium.pdf}
  }
  \subfloat[The Borromean rings]{
  \label{fig:borromean}
  ...
  \caption{A simplified illustration of the \He{6} system together with the Borromean rings. 
  In both figures: If the bond between any two parts is broken, all three fall apart.}
\end{figure}
...
Decreasing the range to $R = \SI{1}{fm}$, we find a wide resonace ($\Gamma > \SI{6}{MeV}$), but this requires large values of $k$ to be included in the basis and thus more scattering states are required to reach convergence. Although it might be possible to vary $R$ and $V_0$ to obtain a resonance closer to experimental data, we do not pursue this method further because of two reasons:
the resonance found is far from the expected value and the slow convergence means we need many basis states which dramatically increases computation time.
...
\begin{table}[b!]
\caption{Experimental \He{6} resonance data from \cite{tunl} and computed values with fitted Woods-Saxon parameters $V_0 = 47.05$ and $V\sub{so}=-7.04$. For the SDI, the optimal interaction strength was $V_0 = \SI{998}{MeV}$ and the contour truncation $k\sub{max} = \SI{2.5}{fm^{-1}}$. All values are in \si{MeV}.}
...
\begin{figure}[H]
   ...
   \caption{Momentum solutions for \He{6}, $2^+$. The resonance is located at $k=\SI{0.241-0.037i}{fm^{-1}}$. We use the fitted Woods-Saxon parameters $V_0 = \SI{47.05}{MeV}$ and $V\sub{so} = \SI{-7.04}{MeV}$. The SDI interaction was used with $V_0=\SI{998}{MeV}$ and the truncation $k\sub{max} = \SI{2.5}{fm^{-1}}$. }
