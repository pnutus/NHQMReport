...
The natural way to proceed is to add another neutron to our two-body \He{5} system and form \He{6}, seen as an alpha particle core with two valence neutrons ($\alpha + n + n$), pictured in \cref{fig:He6}.
...
\begin{table}[b!]
\caption{Experimental \He{6} resonance data from \cite{tunl} and computed values with fitted Woods-Saxon parameters $V_0 = \SI{47.05}{MeV}$ and $V\sub{so}=\SI{-7.04}{MeV}$. For the SDI, the optimal interaction strength was $V_0 = \SI{998}{MeV}$ and the contour truncation $k\sub{max} = \SI{2.5}{fm^{-1}}$. All values are in \si{MeV}.}
