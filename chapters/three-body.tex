...
We now have to take into account the attractive two-body interaction between the neutrons, which allows for bound states in \He{6} where there were none in \He{5}.\todo[inline]{were none or was none? where there in \He{5} was none.}
...
We treat the three-body system as a combination of two identical two-body systems of core and neutron, i.e. two ``one-particle'' systems with the same reduced mass as in \cref{cha:two-body}.
...
\end{eq}
To make the calculations more managable, a \emph{separable} interaction are preferrable since we can write it as
\begin{eq}
  V_l(r_1, r_2) = v_l(r_1)v_l(r_2).
\end{eq}

An arbitrary function $V = V(\vec{r}_1 - \vec{r}_2)$ can be expanded in a \emph{multipole expansion},\todo{cite souhonen somewhere}
...
\end{eq}
using the symmetry around the axis connecting the two particles.
\todo[inline]{This paragraph needs to know why it's here.}
...
\begin{figure}[h]
  ...
  \label{fig:gaussian}
\end{figure}
\todo{var tydligare med att exp skiten är konstant, så man inte tror att vi failat med index}
